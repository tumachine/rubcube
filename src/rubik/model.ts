...
class RubikModel {
  ...
  private check = (side: number, face: number, color: number): boolean => this.getColor(side, face) === color;

  private getColor = (side: number, direction: number): number => this.matrix[side][this.interface[side][direction]];

  private getColorHash = (side: number, direction: number): number => this.colorHashes[this.getColor(side, direction)];
  ...
  solveYellowCenter = () => {
    ...
    const localFind = (row: number, col: number, side: number, operation: Function) => this.baseFind(row, col, side, s.b, operation, localCheck);
